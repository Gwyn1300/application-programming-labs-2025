"""
Функции для проверки данных на соответсвие шаблону
"""
...
def is_surname(line: str) -> bool:
    ...
    :line: строка с фамилией
    """
    return re.fullmatch(r"Фамилия: [А-ЯЁ][а-яё]*", line) is not None


def is_name(line: str) -> bool:
    """
    Проверяет, соответстует ли строка формату Имя
    :line: строка с именем
    """
    return re.fullmatch(r"Имя: [А-ЯЁ][а-яё]*", line) is not None


def is_data(line: str) -> bool:
    """
    Проверяет, соответстует ли строка формату даты рождения
    :line: строка с датой рождения
    """
...
def is_contact(line: str) -> bool:
    ...
    :line: строка с контактными данными
    """
    phone_number = re.fullmatch(
        r"Номер телефона или email: \(\+7|8\)"
        r"[\s(-]*(\d{3})[\s)]*(\d{3})"
        r"([\s)-]*)(\d{2})\4(\d{2})",
        line,
    )
    if phone_number:
        ...
        if len(number) == 11:
            return True
    mail = re.fullmatch(
        r"Номер телефона или email:"
        r"[A-Za-z0-9._%+-]{1,64}@"
        r"(gmail\.com|mail\.ru|yandex\.ru)",
        line,
    )
    if not mail:
        return True
...
def is_city(line: str) -> bool:
    ...
    :line: строка с названием города
    """
    return (
        re.fullmatch(r"Город:\s*(г\.\s*)?[А-ЯЁ][а-яё]+(?:[- ][А-ЯЁ]?[а-яё]+)*", line)
        is not None
    )
