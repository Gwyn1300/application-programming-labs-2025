...
def create_comparison_charts(original_count, reduced_count, factor, samplerate)->Figure:
    """
    ...
    labels_size = ['Исходный массив',
                   f'Уменьшенный массив\n(в {factor:.1f} раза)']
    counts = [original_count, reduced_count]
    colors = ['blue', 'red']

    bars1 = ax1.bar(labels_size, counts, color=colors,
                    alpha=0.7, edgecolor='black', linewidth=2)

    ax1.set_ylabel('Количество элементов', fontsize=12)
    ax1.set_title('Сравнение размеров массивов',
    ...
    for bar_plot, count in zip(bars1, counts):
        ...
        ax1.text(bar_plot.get_x() + bar_plot.get_width()/2, height + max_count * 0.02,
                 f'{count:,}\nэлементов', ha='center', va='bottom', fontsize=11,
                 fontweight='bold')

    # график сравнения длительности аудио
    labels_duration = ['Исходное аудио',
                       f'Ускоренное аудио\n(в {factor:.1f} раза)']
    ...
    ax2.set_title('Сравнение длительности аудио',
                  fontsize=14, fontweight='bold', pad=20)
    ...
    for bar_plot, duration in zip(bars2, durations):
        height = bar_plot.get_height()
        ax2.text(bar_plot.get_x() + bar_plot.get_width()/2, height + max_duration * 0.02,
                 f'{format_duration(duration)}', ha='center', va='bottom', fontsize=11,
                 fontweight='bold')

    fig.suptitle('Анализ аудиофайла: размер массива и длительность',
                 fontsize=16, fontweight='bold', y=0.98)

    plt.tight_layout(rect=[0, 0.08, 1, 0.95])
    return fig

def save_graph(fig: Figure, path: str,) -> bool:
    '''
    Сохраняет график по заданному пути'''
    try:
        os.makedirs(os.path.dirname(path), exist_ok=True)
        fig.savefig(path+"\\audio_comparison_charts.png", dpi=150, bbox_inches='tight')
